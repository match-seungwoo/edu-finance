...
**공식 외교 성명문 195건을 직접 스크래핑·정제**하고, 6개 정량 차원(직설성·동사강도·주어패턴·상호성·완곡어·침묵지도)으로
...
- **데이터**: 30개 사건 × 4개 소스, 실제 수집 183건(정제 후), 30/30 사건 커버
...
### 비판적 검증 (이 프로젝트의 차별점)
- **측정 타당성 자체를 의심**: "동사 강도 최댓값"이 문서 길이와 상관(r=0.94)됨을 발견 → "중국이 가장 강경"이 측정 착시였음을 규명, 1000단어당 밀도로 정규화하니 순위가 뒤집힘(한국 1위).
...
- Exposed a **measurement-validity flaw** — verb-strength max correlated with document length (r=0.94); after density normalization the source ranking reversed — and validated automated results against Korean/Chinese/French source texts
...
2. **수치는 실제 노트북 출력 기준** — 상호성 CN 10.1, 길이 상관 r=0.94, 정규화 후 한국 1위 등은 실제 산출물에 존재. 인터뷰에서 설명할 수 있어야 한다.
3. **표본 한계 명시** — AI 거버넌스 n=21, 프랑스 AI=0 등. "경향"으로 진술, 과일반화 금지.
