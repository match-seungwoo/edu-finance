...
ai.groupby("source").size()''')
    ...
    A(code(r'''# CHECK Step1 — AI 거버넌스가 제대로 골라졌는가 (작은 코퍼스: ~50건)
try:
...
- 표에 행이 **3개(UN/KR/CN)** 만 나오면 정상이다. 프랑스는 데이터가 없다.
    ...
    if KEY:
        A(md("""> 🧑‍🏫 **강사 메모 (정답·실제 값, n=50):**
>
> | 소스 | directness | verb_max | verb_density | mutuality | hedging | we(1인칭) |
> |---|---|---|---|---|---|---|
> | UN | 0.85 | 4.67 | 4.24 | 4.73 | 2.75 | 0.13 |
> | 한국 | 0.80 | 3.36 | 2.20 | **6.66** | 4.10 | 0.20 |
> | 중국 | 0.85 | 6.29 | 4.73 | **10.97** | 3.63 | 0.11 |
>
> 읽는 법: 저당사자성 주제라 **한국·중국 모두 상호성이 UN보다 높다(중국 11.0 ≫ UN 4.7)** — 협력·대화·win-win 어휘.
> 한국은 동사강도가 낮고(verb_max 3.4), 완곡어(hedging 4.10)가 가장 많다 = 신중·완곡한 화법.
> **흔한 실수:** ① `verb_strength_max`(원시)만 보고 "중국 강경" 결론 → 밀도로 봐야 함.
> ② 표에 FR 행을 억지로 만들려다 NaN/에러. ③ 작은 n(50)을 잊고 과한 일반화."""))
...
> |---|---|---|---|---|---|---|
> | 우크라이나 | 高 | 5.74 | 5.52 | 4.57 | 3.28 | 0.80 |
> | 가자 | 高 | 6.47 | 5.51 | 4.54 | 3.50 | 0.79 |
> | **AI 거버넌스** | **低** | **8.15** | **4.02** | 5.16 | 3.47 | 0.84 |
>
> **핵심 반전:** 저당사자성 주제(AI)에서 **상호성이 약 1.3~1.4배(8.15 vs 우크라 5.74 / 가자 6.47)**, **동사강도 밀도는
> 약 3/4 수준(4.02 vs ~5.5)**. 즉 당사자성이 낮을수록 각국은 *더 협력적·덜 강경하게* 말한다.
> 완곡어(hedging)·직설성은 거의 안 변함 — 이건 주제보다 문체/장르 특성일 수 있다(서술형 (b)에서 논의).
> **흔한 실수:** "AI라 상호성이 높다"를 인과로 단정. n=50 + FR 부재라 *경향*까지만 말해야 한다."""))
...
> | 중국 | 33 | 72 | 72 | 0.77 |
>
> **읽는 법(핵심):** escalation 평균(0.74~0.79)만 보면 세 소스가 똑같아 보인다 — *함정*이다.
> **분포**를 보면 신호가 선명하다: **중국은 기회계(benefit 57·opportunity 9)가 위협계의 2배 이상이라
> AI를 '기회'로 프레임**(문서 단위 dominant 명명도 기회 12 / 위협 5). **UN은 challenge/risk/threat/benefit에
> 고루 분포**(위협·관리와 기회를 동시에) — 전형적 *위협 관리* 프레임. 한국은 위협계가 거의 0(safety·challenge 중심)이라
> AI를 '안전·관리' 과제로 신중하게 프레임한다.
> **흔한 실수:** ① escalation 평균이 비슷하니 "차이 없다"고 결론(분포를 봐야 함). ② 프랑스 행을 기대(FR=0).
> ③ n=50·KR 11건을 잊고 "한국은 AI를 위협으로 안 본다"고 단정(표본 부족, *경향*까지만)."""))
...
- 이 *0* 이 무엇을 뜻하는지는 곧 서술형 (d)에서 직접 쓴다.
    ...
    if KEY:
        A(md("""> 🧑‍🏫 **강사 메모 (정답):** **blame=0 · targeted=0 (50건 전부).** 이건 코드 고장이 아니라
...
> **부분 수렴**이다. 고당사자성 주제(우크라이나/가자)에서는 소스들이 모두 '침공·작전' 강경 어휘로 갈렸지만,
> 저당사자성인 AI에서는 **AI 전체 상호성(8.15)이 우크라(5.7)·가자(6.5)보다 높은 협력 어휘로 끌어올랐다**(소스별 UN 4.7 → 중국 11.0) —
> 직접 이해가 걸리지 않은 주제라 모두 '협력·대화·win-win'이라는 *안전한 공통 어휘*로
> 기우는 것으로 보인다. 동사강도(밀도)도 전반적으로 낮아져(4.0 수준 < 고당사자성 ~5.5) 강경함의 편차가 작아졌다.
> 다만 **완전 수렴은 아니다**: 중국은 여전히 상호성·동사강도 max가 높고(mut 11.0, vmax 6.3), 한국은 동사강도가 낮고 완곡어가 가장 많다(hedging 4.1).
> 또 표본이 50건뿐이고 프랑스가 빠져 있어 "수렴"은 *경향* 수준으로만 말할 수 있다.
...
> 1. **표본이 너무 작고 듬성하다(n=50).** AI 거버넌스는 50건뿐이고 한국은 11건, ai01(G7 히로시마)은 전 소스 0건이다.
...
- **소스 비교(n=50):** UN/KR/CN 만. 상호성 중국 10.97 ≫ 한국 6.66 ≫ UN 4.73.
  동사강도 max 중국 6.29 · UN 4.67 · 한국 3.36. (밀도로 보면 한국이 가장 낮음, 2.20.)
- **당사자성 대비:** 상호성 AI 8.15 ≫ 우크라 5.74 / 가자 6.47(약 1.3~1.4배). 동사밀도 AI 4.02 ≪ 5.52 / 5.51.
- **명명 프레임(Step 4a):** 위협계/기회계 단어 — UN 20/38, 한국 1/10, 중국 33/72. escalation 평균은
  0.74~0.79 으로 *거의 같다(평균은 함정)*. 분포로 보면 **중국=기회 편향, UN=위협·관리 혼합, 한국=안전·관리 신중**.
- **차원 비대칭(Step 4b):** `blame_attribution`·`targeted_sentiment` 모두 **0건(50건 전부)**.
...
4. n=50 + FR 부재를 잊고 인과·일반화로 과한 결론. '경향'까지만 허용.
