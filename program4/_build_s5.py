...
오늘의 목표:
1. 가자 코퍼스(147건)를 불러와 **간략한 존재 확인** + **침묵 지도**를 그린다.
...
md("""> **여기서 벌써 Q2의 냄새가 난다.** 한국은 **인도적 위기(gaza04)·ICJ 잠정조치(gaza05)·
> 휴전 붕괴(gaza09)** 에 침묵했다(네 소스 중 유일). 우크라이나 때와 **같은 결**이다 —
> *법적·인도적 변곡점엔 말을 아끼고, 상징적 사건엔 말한다.* 이건 S6에서 깊게 판다."""),

md("""## Step 3 — 가자 전 문서 분석 → 소스별 6차원

`analyze_document` 를 **147개 문서 전부**에 돌린다. spaCy 구문 분석이라 30초~1분.
**S4의 교훈을 잊지 말 것:** `verb_strength_max` 는 *문서 길이에 오염*돼 있다.
그래서 우크라이나 때처럼 **1000단어당 강도동사 밀도(`verb_density`)** 를 같이 만든다."""),
code(r'''# 라이브 분석 — 147개 문서 전부 (한 줄 함수가 6차원을 다 계산)
...
try:
    ...
    # 가자에서는 한국이 중국을 근소하게 앞선다(둘 다 ~12). UN·프랑스는 한참 낮다.
    assert mut["KR"] > mut["UN"] > mut["FR"], "상호성 순위가 이상하다"
    assert mut["KR"] == mut.max() and abs(mut["KR"] - mut["CN"]) < 1.0, "가자 상호성은 한국·중국이 동반 최상위여야 한다"
    assert abs(mut["CN"] - 11.89) < 0.6, "중국 상호성 값이 예상과 다르다"
    assert agg["directness_index"]["CN"] == agg["directness_index"].max(), "중국 직설성이 가장 높아야 한다"
    print("✅ PASS — 가자 소스별 집계 완성")
    print("   상호성: 한국", mut["KR"], "≈ 중국", mut["CN"], "≫ UN", mut["UN"], "> 프랑스", mut["FR"])
    print("   → 가자에선 한국이 중국을 살짝 앞선다(둘 다 ~12). '상호성=중국' 앵커는 우크라이나에서 더 선명히 유지된다.")
except Exception as e:
...
md("""**가자에서도 패턴이 반복된다.**
- 상호성(`mutuality_index`)은 **한국(≈12.13)과 중국(≈11.89)이 동반 최상위** — 둘 다 ~12로 사실상 어깨를 나란히 한다.
  "all parties / dialogue / political settlement / 자제 / 정치적 해결" 프레임을 두 소스 모두 짙게 쓴다.
  (우크라이나에선 중국 ≈11.92 ≫ 한국 ≈5.86 으로 중국이 압도 — **'상호성=중국' 앵커는 우크라이나에서 더 선명하다.**)
- **중국**은 직설성(≈0.84)이 가장 높다. 한국(≈0.81)이 그 뒤를 바짝 따른다.
- **UN·중국**은 `verb_strength_max` 가 높지만(각 6.06 / 6.36) 이는 *문서가 길어서*다 —
  밀도(`verb_density`)로 보면 오히려 한국·UN(≈8.2 / 7.3)이 중국(≈5.1)보다 단호하다(짧고 강함).
- **프랑스**는 짧고 사무적이라 상호성(≈2.20)이 가장 낮다(문체 차이일 수 있음)."""),
...
- **명명(naming_escalation):** **중국이 가장 완곡**(≈0.75) — 지배적 명명어가 *"operation(작전)"* 이다.
  UN이 가장 규정적(≈1.27, "war/genocide"도 등장). 프랑스·한국은 중간(≈1.09 / 0.84).
  → 중국은 *상호성*뿐 아니라 *사건을 부르는 이름*에서도 톤을 낮춘다.
- **귀속(blame):** **UN이 가해자(이스라엘/하마스)를 압도적으로 가장 많이 명시**(named ≈32건). 단,
  UN은 동시에 *행위자를 가린 수동태*(obscured ≈109)도 많아 *비율*(blame_directness≈0.27)은 낮다 —
  "**많이 말하지만 절차적 수동태도 많다**"는 UN 특유의 화법. **한국은 가해자를 단 한 번도 명시하지 않는다**(named 0).
- **대상(most_criticized):** **중국은 이스라엘/팔레스타인을 거의 균등하게 비판**(≈18:20, skew≈0.05) —
  *양면성*이 여기서도 재현된다. UN은 이스라엘 쪽으로 크게 기운다(≈26:8).
- **한 줄:** 가자에서도 *"중국=완곡+균형"* 이 **명명·귀속·대상비판 세 자(尺)에서 동시에** 나온다."""),
...
try:
    ...
    # 중국은 두 주제 모두 상호성을 높게 유지(일관). 우크라이나에선 중국이 단연 1위,
    # 가자에선 한국이 중국을 살짝 추월한다(둘 다 ~12 동반 최상위).
    assert u["mutuality_index"]["CN"] == u["mutuality_index"].max(), "우크라이나 상호성 1위는 중국이어야"
    assert z["mutuality_index"]["CN"] > z["mutuality_index"]["UN"], "가자에서도 중국 상호성은 높은 자리를 유지해야"
    print("✅ PASS — 우크라이나↔가자 소스별 비교 완성")
    print("   관찰: 중국은 두 사건 모두 상호성을 높게 유지(일관). 가자에선 한국이 크게 뛰어 중국과 동반 최상위가 된다.")
except Exception as e:
...
> - **중국**: 두 주제 모두 상호성을 높게 유지(우크라 11.92 → 가자 11.89, 거의 그대로) — *주제 무관 일관된 중립 프레임.* 당사자성 약함(둘 다 제3자).
> - **한국**: 우크라이나(5.86)에서 가자(12.13)로 **상호성이 크게 뛴다(▲6.27)** — 사건에 따라 톤을 가장 크게 바꾸는 *유연한* 소스.
...
md("### (a) 가자 — 소스별 상호성 지수 (한국·중국 ≈12 vs 프랑스 ≈2)"),
...
fig_g.show()
print("→ 가자에선 한국·중국이 동반 최상위(둘 다 ~12). 우크라이나에선 중국이 단연 1위 — '상호성=중국' 앵커는 거기서 더 선명하다.")'''),
...
try:
    ...
    print("   중국은 두 주제 모두 높게 유지(거의 평평). 한국은 우크라이나에서 가자로 크게 뛰어(▲6.27) 가자에선 중국과 동반 최상위.")
except Exception as e:
    print("❌ FAIL —", e, '\n힌트: 빈칸은 "mutuality_index"')'''),
md("""<details><summary>💡 힌트 / 정답</summary>
...
1. 중국은 우크라이나·가자 **둘 다** 상호성을 높게 유지한다(우크라 1위, 가자 한국과 동반 최상위).
   이건 '중립적'이라서일까, 아니면 '두 사건 모두 *당사자가 아니라서*'일까? (Q2 당사자성!)
