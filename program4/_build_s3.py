# -*- coding: utf-8 -*-
"""session3.ipynb 빌더 — 의미 분석: 상호성·완곡어·Claude framing (주제1: 우크라이나)"""
from nb import md, code, save

# ── _build_s1.py 의 SETUP 문자열을 그대로(VERBATIM) 재사용 ──────────────
...
cells = [
md("""# Session 3 — 외교 언어의 *의미*, 두 개의 눈으로 읽기
...
md("""## Step 2 — 왜 규칙 기반과 LLM을 *둘 다* 쓰는가 (3분)
...
md("""## Step 3 — Mutuality Index (상호성 지수) 🎯 **오늘의 핵심**
...
md("""## Step 4 — Hedging Density (완곡어 밀도)
...
md("""### TODO — 소스별 완곡어 밀도 비교 표 만들기
Step 3의 상호성 비교 루프를 그대로 베껴서 `hedging_density` 로 바꾸면 된다."""),
code(r'''# TODO: 빈칸 1개 — hedging_density 를 호출하라
...
code(r'''# 분석 스키마와 프롬프트 (toolkit 의 claude_semantic 과 동일 구조)
SEMANTIC_SCHEMA = {
...
code(r'''# 키 탐색: Colab userdata → 환경변수 순. 없으면 DEMO_MODE.
client = None
DEMO_MODE = True
key = None
try:
...
md("""### 소스별 1건씩 의미 분석해 보기
키가 있으면 진짜 Claude가, 없으면 DEMO 예시가 답한다. 둘 다 **같은 키(스키마)** 로 나온다."""),
code(r'''# 소스별 대표 1건을 골라 Claude(또는 DEMO) 의미 분석
sem_results = {}
...
code(r'''# CHECK Step5 — Claude(또는 DEMO) 출력이 스키마 키를 모두 갖췄는지
try:
...
md("""## Step 6 — Hybrid 교차 비교: 코드의 눈 vs Claude의 눈 🔬
...
빈칸 하나만 채워라."""),
code(r'''# TODO: 빈칸 1개 — Claude 결과(sem_results)에서 mutuality_tone 을 꺼내라
...
code(r'''# CHECK Step6 — 교차 비교가 의미 있게 돌아갔는지
try:
